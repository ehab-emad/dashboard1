...
import { addSellerAddress, deleteSellerAddress, updateSellerAddress } from '../../../../store/reducers/firebasefunctions';
...
const deleteAddress = async(id) => {
  ...
  await  deleteSellerAddress(sellerid,id)
...
const addNewAddress = async(newAddress) => {
  ...
  await addSellerAddress(sellerid,newAddress)
   dispatch(getSeller(sellerid))
  setIsAddAddress(false); // Close the form after adding
};
useEffect(() => {
  if (sellerdata) {
    setAddress(sellerdata.address); // تحديث قائمة العناوين بعد التغيير
  }
}, [sellerdata]);
