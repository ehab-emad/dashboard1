...
const OrderSummary = ({orderData,toggleDetails,index}) => {
  ...
  const onApproveClick = async(order) => {
      ...
      await   dispatch(ChangeOrderStatus({ sellerid, orderid, newStatus }));
  ...
  toggleDetails(index)
      ...
      const onRejectClick = async(order) => {
        const productid=order.productid
        const orderid=order.id
       ...
       {orderData &&  <ReceivingAddress orderData={orderData}/>}  
          ...
          <ActionButtons>
        {
          orderData.status==="pending"?
           <div style={{display:"flex", flexWrap:"wrap",gap:"10px",whiteSpace:"nowrap",width:"100%"}}><ApproveButton
         ...
         <RejectedButton
         onClick={() => onRejectClick(orderData)}
       >
          رفض الطلب
       </RejectedButton></div>: <div style={{display:'none'}}></div>   
        }
