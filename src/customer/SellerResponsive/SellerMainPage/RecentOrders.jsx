...
import {   getproductrented} from '../../../store/reducers/sellerOrdersReducer';
...
const RecentOrders = () => 
  ...
  const {  rented_product } = useSelector((state) => state.seller_orders);
  console.log("rented_product",rented_product)
  const sellerid = localStorage.getItem('sellerId');


  useEffect(() => {

        dispatch( getproductrented(sellerid));
      }, [dispatch]);
  ...
  const memoizedRecentOrders = useMemo(() => {
    ...
    return rented_product 
  }, [rented_product]); // Recalculate only when new_orders changes
    ...
    return (
        <div style={RecentOrderStyle.recentOrders}>
          ...
          {memoizedRecentOrders.slice(0,3).map((product, index) => (
              <RecentProductCard key={index} productdata={product} productid={product.id} />
