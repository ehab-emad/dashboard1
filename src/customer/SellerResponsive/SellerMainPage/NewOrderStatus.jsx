...
import { DeleteorderbyCustomer, getNewOrdersCustomer } from "../../../store/reducers/CustomerOrderreducer";
...
align-self: stretch;
border-radius: 50px;
background: var(--Blue,rgb(141, 31, 31));
...
const NewOrderStatus = ({  user }) => {
 ...
 const toggleProduct = (productId) => {
  ...
  const {  new_orders } = useSelector((state) => state.customer_orders);
  ...
  useEffect(() => {
    ...
    dispatch(getNewOrdersCustomer(customerid ));
  }, [dispatch]);
  ...
  const onApproveClick = async(order) => {
    ...
    await  dispatch(getNewOrdersCustomer(customerid ));
